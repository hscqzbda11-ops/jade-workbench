...
const App = {
  async init() {
    try {
      ...
      // 显示错误提示，避免白屏
      document.body.innerHTML = '<div style="padding:40px 20px;text-align:center;font-family:-apple-system,sans-serif;"><h2 style="font-size:18px;margin-bottom:10px;">加载失败</h2><p style="font-size:14px;color:#888;margin-bottom:20px;">' + (e.message || '未知错误') + '</p><button onclick="location.reload()" style="padding:10px 24px;background:#1a1a1a;color:#fff;border:none;border-radius:20px;font-size:14px;">重新加载</button></div>';
    }
  },
